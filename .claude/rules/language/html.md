...
- `<img>` には `alt` 属性を付与する
  理由: スクリーンリーダーが画像の内容を読み上げられなくなり、視覚障害者のアクセスを阻害する
...
- Boolean 属性は値を省略する (`disabled="disabled"` と書かない)
- 属性値はダブルクォートで囲む
- `id` 属性はページ内で一意にする
...
- 画像には `width` と `height` 属性を指定する
  理由: レイアウトシフト (CLS) を防ぎ、ユーザー体験を保護する
